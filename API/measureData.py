...
class Measurements:
    ...
    def get_stations_data(self, station_ids, measurements, limit):
        # TODO: finish this function
        if station_ids:
            for st_id in station_ids:
                stations_data = iter(self.db.select_station_data(st_id))
                ...
                while counter < int(limit):
                    try:
                        date = datetime.datetime.timestamp(datetime.datetime.now())
                        if counter == sum(
                                1 for line in open(self.prefix + self.to_date(date) + ".csv", 'r', encoding='utf-8')):
                            date += 86400
                        with open(self.prefix + self.to_date(date) + ".csv", 'r', encoding='utf-8') as csv:
                            for line in reversed(csv):
                                counter += 1
